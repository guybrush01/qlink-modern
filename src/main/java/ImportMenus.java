...
class Menu {
  ...
  public String toString(int level) {
    ...
    sb.append(_iIndent);
    sb.append(")\n");
    for (int i = 0; i < _l.size(); i++) sb.append(((Menu) _l.get(i)).toString(level + 1));
    return sb.toString();
  }
  ...
  public String getSQL(int parent) {
    ...
    for (int i = 0; i < _l.size(); i++) sb.append(((Menu) _l.get(i)).getSQL(u));
    return sb.toString();
  }
}
...
public class ImportMenus {
  public static Connection getDBConnection() throws SQLException {
    ...
    return DriverManager.getConnection(
        "jdbc:mysql://localhost/clink?user=clinkuser&password=clink");
  }
