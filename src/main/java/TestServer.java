...
public class TestServer {
  ...
  private static final String DB_URL = "jdbc:mysql://localhost/clink?user=clinkuser&password=clink";
  ...
  private static void saveMenuItem(String string, int id, int i, int j) throws Exception {
    ...
    if (i == 1 && string.endsWith("(+)")) string = string.substring(0, string.length() - 4);
    if (string.startsWith("    ")) string = string.trim();
    else j = 0;
    conn = getDBConnection();
    stmt = conn.createStatement();
    // rs=stmt.executeQuery("SELECT reference_id from toc where reference_id=" + id + " AND type = "
    // + j + " AND code = '" + cost + "' and title='" + string + "'");
    String q =
        "INSERT into toc (reference_id,menu_id, type,cost,title,sort_order,active) VALUES ("
            + id
            + ",0,"
            + j
            + ",'"
    ...
    try {
      ...
      if (stmt.getUpdateCount() == 0) {
        System.err.println(DB_ERROR);
      }
    } catch (Exception e) {
     if (e.getMessage().startsWith(DUPLICATE_PREFIX)) {
        ...
        if (line.length() >= 8) {
          if (line.charAt(7) == 0x20) {
            ...
            if (a.equals("KD") || a.equals("KM")) {
              ...
              save(
                  "Text (" + makeHex(id) + ")",
                  sb1.toString() + sb.toString().replaceAll(cr, "\n"));
              prev = 0;
              next = 0;
              id = 0;
              sb.setLength(0);
