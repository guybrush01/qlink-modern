...
public class RoomManager {
  ...
  private RoomManager() {
   ...
   join(new QHandle("Person "+i),p);
    ...
    // temp people for auditorium ( downt work ) skern
   
   
    for (int i = 1; i <1 ; i++) {
   
   joinAuditorium(new QHandle("Person "+i),p);
  ...
  public synchronized QRoom joinRoom(
    ...
    if (bPublic) room = getPublicRoom(name);
    else room = getPrivateRoom(name);
    if (room.addUser(handle, profile)) return new NormalRoom(room, handle);
    return null;
  }

  // what happens if room has no other users and is public? it shohs there is noome skern
  public synchronized QRoom monitorRoom(
      String name, QHandle handle, ChatProfile profile, boolean bPublic) {
    QRoomDelegate room;
    QSeat user = null;
  ...
  public QRoom joinAuditorium(QHandle handle, ChatProfile profile) {
    ...
    _log.debug("Adding to Auditorium '" + handle + ", " + profile );
  ...
  private QRoomDelegate createRoom(String name, boolean b) {
    ...
    } else room = new RoomDelegate(name, b, false);
    return room;
  }
