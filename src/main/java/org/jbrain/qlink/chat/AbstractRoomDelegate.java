...
public abstract class AbstractRoomDelegate implements QRoomDelegate {
  ...
  public int getPopulation() {
    if (_log.isDebugEnabled()) _log.debug("Population of '" + _sName + "' is " + _htUsers.size());
  ...
  public void leave(QHandle handle) {
    ...
    if (_htAdmins.get(handle.getKey()) != null) _htAdmins.remove(handle.getKey());
  }
  ...
  // this must be sycnchronized on the instance, as the facades syncrhonize on the room instance to
      ...
      if (cmd.startsWith("gag") && info.getProfile().isEnabled("room.user.gag")) {
        ...
        send(
            new ChatEvent(
                ...
                -1,
                SYS_NAME,
                "You are violating the Q-Link Terms of Service. Please STOP NOW!"));
        ...
        if (st.hasMoreTokens()) {
          ...
          sNum = st.nextToken();
          if (st.hasMoreTokens()) sSize = st.nextToken();
          try {
