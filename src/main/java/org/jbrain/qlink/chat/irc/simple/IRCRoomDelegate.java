...
public class IRCRoomDelegate extends AbstractRoomDelegate {
  ...
  static {
    SERVER_HOST = QConfig.getInstance().getString("qlink.chat.irc.server", "chat.freenode.net");
  ...
  private static Observer _stateListener =
      new Observer() {
    ...
    if (text != null && text.length() > 0) {
      ...
      if (cmd.startsWith("topic")) {
        ...
        if (msg != null) {
          ...
          if (_bJoined) sendIRCCommand(new TopicCommand(_sChannel, msg));
          else error = "Error: not connected to server";
        }
      } else {
  ...
  protected void send(RoomEvent event) {
    if (event instanceof JoinEvent) {
      if (((JoinEvent) event).getType() == JoinEvent.EVENT_JOIN) {
        ...
        processEvent(
            new ChatEvent(this, -1, "", "*" + ((JoinEvent) event).getName() + " enters the room"));
        if (_bJoined) sendJoinInfo(((JoinEvent) event).getName());
      } else {
        // we need to send a "Joined..." messahge to IRC and to the channel.
        // processEvent(new JoinEvent(this,JoinEvent.EVENT_LEAVE,22,((JoinEvent)event).getName()));
        processEvent(
            new ChatEvent(this, -1, "", "*" + ((JoinEvent) event).getName() + " leaves the room"));
        if (_bJoined)
          sendIRCCommand(
              new MessageCommand(_sChannel, "Left QRoom: " + ((JoinEvent) event).getName()));
      }
      ...
      if (_bJoined) {
        ...
        if (name == null || name.equals(""))
          ...
          sendIRCCommand(
              new MessageCommand(_sChannel, "{" + name + "} " + ((ChatEvent) event).getText()));
      }
    ...
    if (arg1 instanceof CtcpMessage) {
      ...
      if (room != null) {
        if (cmd.getUser().equals(SERVER_NICK)) {
          ...
          room.processEvent(
              new JoinEvent(room, JoinEvent.EVENT_JOIN, 23, fix(cmd.getUser().toString())));
  ...
  private synchronized void connect() {
    _log.debug("Connecting to IRC server " + SERVER_HOST + " room " + _sChannel);
    if (_ircConn == null && !_bConnected) {
      ...
      AutoRegister autoReg =
          new AutoRegister(_ircConn, SERVER_NICK, "qlink", "QuantumLink RELOADED");
