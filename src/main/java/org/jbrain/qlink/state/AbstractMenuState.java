...
public abstract class AbstractMenuState extends AbstractState {
  ...
  protected void sendMessageList() throws IOException {
    ...
    if (size == 0) {
		...
		_session.send(new PostingItem("No News (is good news) press F5 ", PostingItem.NEXT));
		for (int j=0; j < 10; j++){
		 _session.send(new PostingItem("\u0005", PostingItem.NEXT));
		 }
		
		 _session.send(new PostingItem("", PostingItem.LAST));
 ...
 protected void sendSurchList() throws IOException {
	...
	_session.send(new InitDataSend(0, 0, 0, 0, 0));
		...
		_session.send(new InitDataSend(0, 0, 0, 0, 0));
		_session.send(new FileText("fount "+ size+"\n", false));
      while (i < 4 && _iLines < size - 1) {
        i++;
        _iLines++;
        m = (MessageEntry) _alMessages.get(size - _iLines);
