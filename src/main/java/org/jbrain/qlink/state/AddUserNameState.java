...
public class AddUserNameState extends AbstractAccountState {
  ...
  private DialogCallBack _addAccountCallBack =
      new DialogCallBack() {
        /* (non-Javadoc)
          ...
          if (a instanceof ZA) {
            ...
            if (validateNewAccount((EntryDialog) d, text)) {
              ...
              if (id > -1) {
                ...
                _session.send(
                    ((EntryDialog) d).getSuccessResponse("Creating user name '" + handle + "'."));
                // adding name.
                _log.debug("Adding new account to disk");
                _session.send(new InitDataSend(0, 0, 0));
                _session.send(
                    new FileText("User name '" + handle + "' successfully created.", false));
  ...
  static {
    ...
    _addAccountDialog.addText(
        "Please choose a screen name and type it in.  Your screen name must contain at least 3 non-space characters and can be up to 10 characters in length.\n\n  Press F5 to cancel operation.");
  }
  ...
  public void activate() throws IOException {
    ...
    if (l.size() < 5) {
      ...
      EntryDialogState state =
          new EntryDialogState(_session, _addAccountDialog, _addAccountCallBack);
      state.activate();
    } else {
      _session.send(new InitDataSend(0, 0, 0));
      _session.send(
