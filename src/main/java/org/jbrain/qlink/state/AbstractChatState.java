...
public abstract class AbstractChatState extends AbstractState {
  ...
  class QueuedChatEventListener implements RoomEventListener {
    ...
    public synchronized void resume() {
      ...
      while (_alQueue.size() > 0) {
        ...
        else if (e instanceof QuestionStateEvent
            && ((QuestionStateEvent) e).getType() == QuestionStateEvent.ACCEPTING_QUESTIONS)
          acceptingQuestions((QuestionStateEvent) e);
        else if (e instanceof QuestionStateEvent
            && ((QuestionStateEvent) e).getType() == QuestionStateEvent.NOT_ACCEPTING_QUESTIONS)
          rejectingQuestions((QuestionStateEvent) e);
      }
    }

    public void userSaid(ChatEvent event) {
      if (_bSuspend) _alQueue.add(event);
      ...
      if (text.length() > 2) {
        ...
        if (cmd.startsWith("sysmsg") && _session.isStaff()) {//
            ...
            if (_mgr.getUserLocation(handle) != null) {
              ...
              _session
                  ...
                  .sendToUser(
                      handle,
                      new Toss(
                          "You have violated the TOS and your session has been terminated "
                              + (char) 0xff
                              + (char) 0xff
                              + "Press F5 to return to BASIC"));
          ...
          if (seat != null && !seat.isInGame()) {
            ...
            if (name != null) {
              ...
              if (info != null && info.getUserID() == _session.getUserID()) {
                ...
                else error = "Could not change user name";
              } else {
          ...
          if (name != null && msg != null) {
            ...
            if (UserManager.getAccount(handle) == null) error = "User not valid";
            else if (!_session.getServer().isUserOnline(handle)) error = "User not online";
  ...
  protected void showSeats(QSeat[] seats, boolean bRoomChange) {
    ...
    if (mySeat > -1) _session.send(new AddUserToRoom(mySeat, _session.getHandle().toString()));
    else _session.send(new AddUserToRoom(23, _session.getHandle().toString()));
    _listener.resume();
  ...
  protected void showSeatsAudit(QSeat[] seats, boolean bRoomChange) {
    ...
    _log.debug("Sending Audit seat information");
    for (int i = 0, size = seats.length; i < size; i++) {
      user = seats[i];
      if (!_session.getHandle().equals(user.getHandle()))
        if (bRoomChange) _session.send(new CL(user.getSeatID(), user.getHandle().toString()));
    ...
    if (mySeat > -1) _session.send(new EnterTheAuditorium(mySeat, _session.getHandle().toString()));
    else _session.send(new EnterTheAuditorium(23, _session.getHandle().toString()));
    _listener.resume();
    if (!bRoomChange && checkEmail()) _session.send(new NewMail());
  }
  ...
  protected ChatProfile getProfile(AccountInfo info) {
    ChatProfile profile;
    if (info.isStaff()) profile = _staffProfile;
    else profile = _normalProfile;
    return profile;
  }
}
