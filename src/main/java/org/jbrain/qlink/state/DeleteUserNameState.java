...
public class DeleteUserNameState extends AbstractState {
  private static Logger _log = LogManager.getLogger(DeleteUserNameState.class);
  private static YesNoDialog _accountDialog;
  private int _iSelectedID;
  private DialogCallBack _accountCallBack =
      new DialogCallBack() {
        ...
        public boolean handleResponse(AbstractDialog d, Action a) throws IOException {
          ...
          if (a instanceof DialogYes) {
            ...
            _session.send(
                ((YesNoDialog) d)
                    .getSuccessResponse("Removing user name '" + info.getHandle() + "'"));
            if (_log.isDebugEnabled())
              _log.debug(
                  "Removing user name '" + info.getHandle() + "' in slot: " + (_iSelectedID + 1));
  ...
  static {
    ...
    _accountDialog.addText(
        "Are you sure you wish to remove this screen name? Select 'yes' or 'no'.");
  }
  ...
  private void showAccountList() throws IOException {
    ...
    if (!_lAccounts.isEmpty()) {
      _session.send(
          new MenuItem(
              0,
              "Please select user name to remove",
              MenuItem.HEADING,
      ...
      for (int i = 0; i < _lAccounts.size(); i++) {
        ...
        _session.send(
            new MenuItem(
                i + DepartmentMenu.RESERVED_MENU_REF_ID_MIN,
                "    " + info.getHandle(),
                MenuItem.MENU_DATA,
                MenuItem.COST_NO_CHARGE,
                i + 1 == _lAccounts.size()));
  ...
  public boolean execute(Action a) throws IOException {
    ...
    if (a instanceof SelectMenuItem) {
      ...
      try {
        ...
        _session.send(
            new FileText(
                "An error occured while removing user name '"
                    + info.getHandle()
