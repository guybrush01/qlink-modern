  ...
  private void migrateAccounts(EntryDialog d, QHandle handle) throws IOException {
    ...
    if (info != null) {
      ...
      if (info.getUserID() == _session.getUserID()) {
        ...
        _session.send(
            d.getErrorResponse(
                "You cannot migrate accounts to the disk you are terminating.  Please select a user name from a different disk."));
      } else {
        ...
        if (oldList.size() + newList.size() > 5) {
          ...
          _session.send(
              d.getErrorResponse(
                  "This disk contains "
                      ...
                      + "' has already created "
                      + newList.size()
                      + " accounts, which would exceed the user name maximum (5).  Please delete user names from one or both disks and try again."));
        } else {
          ...
          try {
            ...
            try {
              ...
              _session.send(
                  new FileText(
                      "Accounts and user names successfully migrated to '"
                          + handle
                          + "'.  The system will now disconnect.",
                      true));
              try {
                Thread.sleep(15000);
          ...
          if (!rc) {
            _session.send(
                d.getErrorResponse(
                    "An error occurred while migrating the user names.  Please contact customer service."));
            _log.debug("Account migration error");
            _session.send(new InitDataSend(0, 0, 0));
            _session.send(
                new FileText("Accounts and user name migration was not successful..", true));
          }
        }
      }
    } else {
      ...
      _session.send(
          d.getErrorResponse(
              "We're sorry, but user name '"
                  + handle
                  + "' is not a valid user.  Please select another name."));
    }
  }
}
