  ...
  public boolean execute(Action a) throws IOException {
    ...
    if (a instanceof RequestGame) {
     ...
     //enter not Auditorium
      String name = ((EnterPrivateRoom) a).getRoom();
      
       _log.debug("Entering private room "+name);
    ...
    if(name.toLowerCase().equals("auditorium")&&! _session.isStaff()){
      ...
      if (info == null) {
        ...
        if (room == null) {
          ...
          _log.debug(
              "LOCATE Success:  User '" + handle + "' is in public room '" + room.getName() + "'");
          _session.send(new LocateUserText(room.getName()));
        } else {
          _log.debug(
              "LOCATE Success:  User '" + handle + "' is in private room '" + room.getName() + "'");
      ...
      synchronized (_room) {
       ...
       sendRoomInfo(seats);//send header info .................................
      ...
      _log.debug(
          _session.getHandle()
              + " wants system to pick partners for "
              + ((FindPartners) a).getTitle());
...
/** *///SKERN
 private void sendAutoText() {
  ...
  private void enterAuditorium() {
     _log.debug("Enter the Auditorium ");
 
    //todo the gui enter this romm dont epper in outer cilds SKERN
   _session.send(new DrawsTopline("Auditorium"));//New Head old menue still in lobby
   enterRoom("Auditorium",false);//goto privatroom Auditorium
   ...
   if (_room.canTalk()) _session.send(new AcceptingQuestions());//questions or not
     ...
     _log.debug("Please Add Master in Auditorium");
