...
public class GetUserInfoState extends AbstractState {
  ...
  private DialogCallBack _cityCallBack =
      new DialogCallBack() {
        public boolean handleResponse(AbstractDialog d, Action a) throws IOException {
          _log.debug("We received " + a.getName() + " from entry dialog");
          if (a instanceof ZA) {
        ...
        public boolean handleResponse(AbstractDialog d, Action a) throws IOException {
          ...
          if (a instanceof ZA) {
            ...
            EntryDialogState state =
                new EntryDialogState(_session, _countryDialog, _countryCallBack);
            state.activate();
            return true;
          }
          return false;
      ...
      new DialogCallBack() {
        public boolean handleResponse(AbstractDialog d, Action a) throws IOException {
          ...
          if (a instanceof ZA) {
            ...
            _verifyDialog =
                new EntryDialog("DATAOK", YesNoDialog.TYPE_LOGIN, YesNoDialog.FORMAT_NONE);
        ...
        public boolean handleResponse(AbstractDialog d, Action a) throws IOException {
          ...
          if (a instanceof ZA) {
            if (((ZA) a).getResponse().trim().toLowerCase().startsWith("y")) {
              ...
              _session.send(
                  ((EntryDialog) d).getErrorResponse("Answer the question with 'yes' or 'no'"));
            }
          } else if (a instanceof D2) {
            QState state;
            if (_bGoodData) {
            ...
            } else state = new EntryDialogState(_session, _nameDialog, _nameCallBack);
            state.activate();
          }
          return false;
        }
