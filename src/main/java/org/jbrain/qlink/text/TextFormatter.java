  ...
  private void addParagraph(String string, int pos) {
    ...
    if (pos + _iWidth >= string.length()) {
      ...
      if (pos == start) {
        ...
        int pos2 = pos;
        while (pos2 > start && string.charAt(pos2) == ' ') pos2--;
        if (pos2 != start) addLine(string.substring(start, pos2 + 1));
        else {
          // case '    longline......'
      ...
      // walk forward...
      while (pos < string.length() && string.charAt(pos) == ' ') pos++;
      if (pos < string.length()) addParagraph(string, pos);
    }
  }
